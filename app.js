...
function createCard(element) {
    ...
    $h2 = $(`<h2 class="card-title">${element.author}</h2>`);
    $cardHeader.append($h2);

    $cardBody = $('<div class="card-body"></div>');
    $card.append($cardBody);

    $img = $(`<img src="${element.download_url}" class="card-img-top img-fluid rounded" alt="">`);
    $cardBody.append($img);
    ...
    $thumbsUp.click(function () {
        ...
        $imgModal = $(`<img class="img-fluid" src=${element.download_url} alt="">`)
...
$likedImgNav.click(function () {
    ...
    likedImg.forEach(element => {
        let $imgModal = $(`<img class="img-fluid" src=${element.download_url} alt="">`)
...
$(window).on("scroll", function () {
    ...
    if(((scrollHeight - 300) >= scrollPos) / scrollHeight == 0) {
        count++;
        $.get(`https://picsum.photos/v2/list?page=${count+10}&limit=1`, function (data) {
